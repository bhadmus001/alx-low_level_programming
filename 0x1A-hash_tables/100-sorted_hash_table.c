...
/**
 * shash_table_create - creates a shash tables with a given size
...
shash_table_t *shash_table_create(unsigned long int size)
{
	...
	if (table == NULL)
		return (NULL);
	array = malloc(sizeof(shash_table_t *) * size);
	if (array == NULL)
		return (NULL);
	for (i = 0; i < size; i++)
		array[i] = NULL;
...
/**
 * add_n_shash - adds node at the beginning of a shash at a given index
